...
| auth | next-auth/clerk/lucia/auth0 | 자체 crypto 금지, 서버 인가, httpOnly 세션 (보안 크리티컬) |
| payments | stripe/toss/paddle | 서버 금액결정, 웹훅 서명검증, 멱등성 (보안 크리티컬) |
| hosting | cloudflare/vercel/netlify | 시크릿은 플랫폼, 환경 분리, IaC |
| email | resend/sendgrid/postmark | 서버사이드 전송, 키 비노출, 도메인 인증 |
| ai | anthropic/openai | 키 서버사이드, 비용/캐싱, 출력 검증 |

generic 규칙은 `claude-rules/capabilities/<cap>.md`로 신설 (총 11개). auth/payments는 보안 크리티컬이라 우선 포함, hosting/email/ai는 감지 테이블의 모든 capability가 generic 바닥을 갖도록 메움. **generic 규칙은 named로 안 덮인 capability에만 적용**(Tier1 fallback)되며, 한 capability에 named+generic 스택이 공존하면(예: drizzle+prisma) 둘 다 적용한다.
...
**모드 (engine 하나를 모드로 노출):**
- `apply` (기본): vendoring + CLAUDE.md 합성 + `adopt-report.md`
- `dry-run`: 감지+갭만, **아무것도 안 씀** (적용 전 read-only 점검)
- `verify`: 설치 상태 검증(예상 규칙/관리블록 존재), 안 씀 (적용 후 검증)

점검/검증은 별도 **`inspect-project`** 스킬로 노출(`--dry-run`/`--verify` 래핑), 적용은 **`adopt-existing-project`** 스킬.
...
- **스택 추천기(recommender) — 별도 스킬+플랜 (설계 메모)**: 거버넌스(how)와 분리된 *추천(which)* 기능. capability가 **아예 비어 있을 때**(예: analytics 0개)는 named·generic 둘 다 다스릴 대상이 없으므로, 프로젝트 특성으로 후보 스택을 추천한다. 입력 가중치: 플랫폼(web/native/edge) · 서비스 지역(예: KR) · 예산 · 성숙도/평판 · **기존 스택 호환**(이미 Cloudflare면 CF-native 우대). **운영중 프로젝트는 이미 쓰는 capability는 교체 추천하지 않는다**(리스크) — 비어 있는 capability에만 추천. `inspect`의 갭 출력(빈 capability)을 입력으로 소비하는 구조. 평판/가격/지역 성능은 리서치가 필요해 P1/P2 코어와 결이 달라 분리. P4(고객 repo)에서 가치 큼.
