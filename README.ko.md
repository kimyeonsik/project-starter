...
자연어 외에, 스킬/엔진을 두 가지로 호출할 수 있습니다:

**Claude Code 안에서 — 슬래시 명령** (`<scope>/.claude/commands/`에 설치):
...
**터미널에서 — `project-starter` CLI** (클론에서 `npm link` 후, 또는 `node scripts/cli.mjs`):

```bash
project-starter adopt --dry-run      # 대상 repo에서 실행 (또는 PROJECT_ROOT 지정)
project-starter inspect | verify | update | install | secrets | version | help
```

CLI는 순수 Node라 **AI 토큰·Claude 구독이 필요 없습니다** — CI/자동화에 유용. 신규 부트스트랩은 AI 주도라 `/bootstrap` 슬래시 명령은 있어도 터미널 명령은 없습니다.
