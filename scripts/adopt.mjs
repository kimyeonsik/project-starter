...
// 리포트 푸터 — 어느 버전이 생성했는지 기록.
const reportFooter = () => `\n---\n_Generated by project-starter v${VERSION}_\n`;
...
export function runAdopt(repoDir, opts = {}) {
  ...
  const report = renderReport(gaps, detected) + reportFooter();
  ...
  const postReport = renderReport(postGaps, detected) + reportFooter();
...
if (isMain) {
  ...
  info(`project-starter v${VERSION} · adopt (${mode}): ${repoDir}`);
